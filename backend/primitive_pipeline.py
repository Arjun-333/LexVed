...
MODEL_INDEX_MAP = {
    "multi-qa-mpnet-base-cos-v1":  "legal-hybrid-rag",
    "multi-qa-MiniLM-L6-cos-v1":  "prim-minilm-index",
    "multi-qa-distilbert-cos-v1": "prim-distilbert-index",
    "BGE-M3":                     "prim-bge-m3-index",
}
...
def get_or_create_index(name: str, dim: int):
    ...
    if name not in existing:
        ...
        print(f"[Primitive] Index '{name}' created (dim={dim})")
    else:
        print(f"[Primitive] Index '{name}' exists.")
...
def embed_and_upsert(chunks, embedder, model_name, index, batch_size=96):
    # Check if already populated
    stats = index.describe_index_stats()
    existing = stats.get("total_vector_count", 0)
    if existing >= len(chunks) * 0.9:
        print(f"[Primitive] Index already has {existing} vectors — skipping upsert.")
        emb_time = 0.0
        # Still need vectors for in-pipeline similarity (fetched from index at query time)
    ...
    for i, vec in enumerate(tqdm(vecs, desc="Upserting to Pinecone")):
        batch.append({"id": str(i), "values": vec.tolist(),
                      "metadata": {
                          "text": chunks[i]["text"], 
        ...
        if len(batch) >= batch_size:
            index.upsert(vectors=batch); batch = []
    if batch: index.upsert(vectors=batch)
    print(f"[Primitive] {len(vecs)} vectors upserted.")
...
def run_evaluation(model_name, embedder, index, chunks, queries, gts, emb_time, index_size):
    ...
    for q in tqdm(queries, desc=f"Evaluating {model_name}"):
        ...
        t0 = time.time()
        res = index.query(vector=q_vec.tolist(), top_k=5, include_metadata=True)
        ret = [m["metadata"].get("text", "") for m in res["matches"]]
        rt = time.time() - t0
...
def run_primitive_pipeline(model_choice="1", api_key=None):
    ...
    _write_status({"status": "processing", "progress": f"Embedding & upserting {len(chunks)} chunks to Pinecone...", "embedding": model_name})
