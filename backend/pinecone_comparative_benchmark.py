...
if synthetic_path is not None:
    ...
    GOLD_CHUNKS = [
        ...
        "(e) The principles of actus [REDACTED_NAME] gravabit are not attracted in these cases, since the petitioners and other candidates in the Rank List dated 11.09.2013 have obtained all the vacancies that would have been entitled to them even if there were no litigations or interim orders. (f) The views and conclusions in Kerala Public Service Commission v. Dr. [REDACTED_NAME] (1977 KLT 818), that a candidate in a rank list does not get an indefeasible right to be appointed, is certainly good law and such position does not change even after the coming into force of the PSC Procedure Rules because these Rules only offer the candidates, included in a rank list, a statutory right to be considered against the vacancies available and reported during the life time of such list, Aneesh Kumar V. S. vs The State Of Kerala on 24 April, 2020 Indian Kanoon - http://indiankanoon.org/doc/58045573/ 7 but not to be appointed. That brings us to the question if these original petitions need to be placed before the Division Bench for final resolution or whether we would be justified in issuing judgment. Since the relief prayed for in these original petitions, except O.P. (KAT) no.",
...
def evaluate_pipeline(pipeline_type):
    ...
    def is_match(ret_doc, gold_doc):
        ...
        if jaccard >= 0.60:
            return True
        # 3. Fallback: Embedding similarity
        try:
