...
[Agent Synthesis]
  ↓
[Compliance Auditor] -- (Fails Audit?) -> Loops back to Agent Node with feedback
  ↓ (Passes Audit)
Answer
```
...
## 10. Advanced Agentic Enhancements (Latest Releases)

### 1. Dynamic Query Decomposition (Sub-Query Extraction)
* **Goal**: Solve complex legal queries with multiple parts or different laws/cases.
* **Implementation**: Large queries containing multiple thoughts are routed to Llama-3.1-8B-Instant. The LLM extracts 2-3 target search-friendly sub-queries. The retriever runs dense/sparse RRF searches concurrently for all sub-queries, merges and deduplicates results, and uses the CrossEncoder to rerank candidates against the original question.
* **Benefits**: Boosts retrieval recall on compound questions.

### 2. Conversational Self-Correction (Compliance Audit Loop)
* **Goal**: Eliminate hallucinations and verify factual consistency before returning replies.
* **Implementation**: We added a `Compliance Auditor` node to the LangGraph state machine. Every draft generated by the agent is cross-examined against the retrieved text. If factual contradictions or unsupported assumptions are found, the auditor loops back to the agent with explicit corrections. The agent uses this feedback to revise its answer.
* **Benefits**: Institutional-grade accuracy with zero hallucinations.

### 3. Interactive Split-Screen PDF Viewer & Highlight Panel
* **Goal**: Provide loss-free verification of cited case laws within the chat container.
* **Implementation**: When clicking on any case citation, the frontend divides the viewport (60% Chat, 40% PDF side-panel). An iframe loads the source PDF at the exact page number. A bottom highlight panel renders the matching text context.
* **Benefits**: Premium UI/UX that speeds up legal citation auditing.
