...
from qdrant_pipeline import run_primitive_pipeline as run_qdrant_pipeline
...
try:
    import importlib; importlib.import_module("FlagEmbedding")
    MODELS.append(("6", "BGE-M3"))
except: print("[Runner] BGE-M3 skipped (FlagEmbedding not installed)")
...
print("  Qdrant + Groq llama-3.1-8b-instant")
print("="*70)
...
for choice, label in MODELS:
    print(f"\n{'─'*60}\n  Running: {label}\n{'─'*60}")
    t0 = time.time()
    df = run_qdrant_pipeline(model_choice=choice)
    elapsed = time.time() - t0
    if df is not None:
        results[label] = df
        # Read system_info from status file
        try:
            rpt = json.loads((BACKEND_DIR / "qdrant_evaluation_results.json").read_text())
...
# ── Generate PDF ──────────────────────────────────────────────────────
out_pdf = BACKEND_DIR / f"qdrant_audit_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
...
story.append(Paragraph(
    ...
    "Qdrant Pipeline · Groq LLM Judge",
    csm
))
